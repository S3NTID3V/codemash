    ...
    if user_input:
        ...
        if gemini_client:
            ...
            if "what's next" in user_input.lower():
                handle_task_generation(gemini_client, context)
            elif user_input.lower().startswith("done"):
                handle_task_verification(user_input, gemini_client, context)
            elif "summarize repo" in user_input.lower():
                handle_repo_summary(gemini_client, repo_path)
            else:
...
def build_ai_context():
    """Builds a string of the current project context to pass to the AI."""
    context_parts = []
    context_parts.append("You are an expert AI project manager. Your role is to help the user manage their software development projects by generating tasks, verifying completion, and providing insights.")
    ...
    if st.session_state.logs:
        ...
        for log in st.session_state.logs[-5:]:
            context_parts.append(f"- {log['timestamp']}: {log['event']} - {log.get('details', 'No details')}")

    # Add recent chat history
    if st.session_state.chat_history:
        context_parts.append("\n--- Recent Conversation (Last 4 Messages) ---")
        for author, message in st.session_state.chat_history[-4:]:
             context_parts.append(f"{author.capitalize()}: {message}")

    return "\n".join(context_parts)

def handle_task_generation(gemini_client, context):
    """
    ...
    Based on the provided project context, generate the next single, logical task to move the project forward.
    ...
    with st.spinner("AI is thinking of the next task..."):
        ai_response = gemini_client.generate_response(prompt)
...
def handle_repo_summary(gemini_client, repo_path):
    """Handles the 'summarize repo' command."""
    if not repo_path or not os.path.isdir(repo_path):
        st.session_state.chat_history.append(("assistant", "Repository path is not configured or is invalid. Please set it in the sidebar."))
        with st.chat_message("assistant"):
            st.markdown("Repository path is not configured or is invalid. Please set it in the sidebar.")
        return

    with st.spinner("Scanning repository and generating summary..."):
        file_structure = []
        for root, dirs, files in os.walk(repo_path):
            ...
            indent = "  " * level
            if level == 0:
                 file_structure.append(f"{os.path.basename(repo_path)}/")
            else:
        ...
        if not file_structure:
            summary = "The repository is empty or contains only hidden files."
        else:
            file_structure_str = "\n".join(file_structure)
            prompt = f"""
            Based on the following file structure, provide a high-level summary of the project.
            What is the likely technology stack and purpose of this repository?
    ...
    st.session_state.chat_history.append(("assistant", f"**Repository Summary:**\n{summary}"))
    with st.chat_message("assistant"):
        st.markdown(f"**Repository Summary:**\n{summary}")
    log_event("Repo Summary Generated", {"path": repo_path})
    save_project_data()

def handle_task_verification(user_input, gemini_client, context):
    """
    ...
    prompt = f"""
    A task was marked as completed. Based on the project context and the user's summary, please verify if the task is truly done.

    {context}

    Current Task Description: {current_task['description']}
    ...
    with st.spinner("AI is verifying the task..."):
        ai_response = gemini_client.generate_response(prompt)
